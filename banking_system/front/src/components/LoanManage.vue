                    ...
                    <v-card outlined class="my-4 px-4" v-if="idxOut==2">
                      ...
                      <v-form v-model="formCheck_1">
                        <v-row>
                          ...
                          <v-col cols="4">
                            <v-text-field
                              v-model="inputDataLoanPayment.loan_payment_Serial_number"
                              counter="18"
                              label=" 发放序号"
                              outlined
                              dense
                        ...
                        <template v-slot:default>
                          <thead>
                            <tr>
                              <th class="text-left">Index</th>
                              <th class="text-left">Loan ID</th>
                              <th class="text-left">Loan Payment Serial Number</th>
                          ...
                          <tbody>
                            <tr v-for="(ds, index) in receiveData.data" :key="index">
                              <td>{{ index }}</td>
                              <td>{{ ds.loan_id }}</td>
                              <td>{{ ds.loan_payment_Serial_number }}</td> 
...
export default {
  ...
  data: () => ({
    ...
    inputDataLoanPayment: {
      loanID: "",
      loan_payment_Serial_number: "",/*这里在以后迭代版本时随着将贷款发放序号设置为自增主键，实现无需让用户输入 */
      date: "",
      money: ""
    },
    inputDataOwner: {
  ...
  computed: {
    ...
    postData: function() {
      return {
        ...
        loan_payment_Serial_number: this.inputDataLoanPayment.loan_payment_Serial_number,
