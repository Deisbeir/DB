                ...
                if modify_data['c_identity_code'] == '':
                    ...
                    if result['status']:
                        ...
                        if customers_check is not None:
                            result['message'] = '目标客户ID与其他已存在的客户ID冲突，无法更改'
                            result['status'] = False
                    if result['status']:
                        # modify
                        for c in customers:
        ...
        if args['tab_out'] == '0':
            if args['tab_in'] == '0':
                ...
                if result['status']:
                    for a_id in accounts_id:
                        ...
                        #后续迭代可考虑设计在更改所有人页面时可直接更改用户ID，这里并不算很完整，需要在其他地方也添加相应改动
                        # if args['m_account_customer_id']!='':
                        #    acc = Account.query.filter_by(a_code=a_id).first()
                        #    acc.c_identity_code = args['m_account_customer_id']
                        db.session.commit()
                    result[
                        'message'] = '更改成功'
            else:
            ...
            if args['tab_in'] == '0':
                ...
                if result['status']:
                    ...
                    if account is None:
                        result['status'] = False
                        result['message'] = '账户类型不匹配'

                if result['status']:
                    # check consistency on the open bank
                    account_info = Account.query.filter_by(
                        a_code=args['i_account_id']).first()
                    a_open_bank = account_info.a_open_bank
                    if a_open_bank != args['i_account_open_bank']:
                        result['status'] = False
                        result['message'] = "银行不匹配"
...
class LoanManagement(Resource):
    ...
    parser.add_argument('loan_payment_Serial_number', type=str, required=False)
    ...
    def post(self):
        ...
        convert_dict_loan_payment = {
            'loan_id': 'lp_l_code',
            'loan_payment_Serial_number': 'lp_Serialnumber',
            ...
            if args['tab_in'] == '0':
                ...
                if result['status']:
                    loan_payment = LoanPayment.query.filter_by(
                        lp_Serialnumber=args['loan_payment_Serial_number'],
                        lp_l_code=args['loan_id']).first()
                    if loan_payment is not None:
                        result['status'] = False
                        result['message'] = '贷款序列号不正确，本次贷款已发放'
                max_money = 0
                if result['status']:
                    loan = Loan.query.filter_by(l_code=args['loan_id']).first()
                    ...
                    if math.isclose(l_money, lp_money):
                        result['status'] = False
                        result[
                            'message'] = '贷款发放已完成，无法插入'
                    #else:
                    #max_money = l_money - lp_money
                max_money = l_money - lp_money   
                if result['status']:
                    if float(args['money']) > max_money:
                        #args['money'] = str(max_money)，这种实现是自动将超出的发放额截断，只取到发放完贷款的金额数为止。但本次不用这个实现，而采用：发放额超出的时候，以反馈信息的方式提醒用户重新设定发放额。
                        result['status'] = False
                        result[
                            'message'] = '本次贷款发放超过剩余所需发放的金额，无法插入'
                if result['status']:
                    info = {
